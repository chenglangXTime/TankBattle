(() => {
  ...
  function resetGame(startAfterReset = false) {
    ...
    blueTank = createTank("blue", {
      ...
      x: WORLD.width - 270,
      facing: -1,
      zone: [WORLD.width * 0.5 + 90, WORLD.width - 90],
    });
  ...
  function terrainY(x) {
    const base = WORLD.height - 145;
    return (
      base +
      34 * Math.sin(x / 145) +
      22 * Math.sin(x / 82 + 1.4) +
      16 * Math.cos(x / 260 - 0.8)
    );
  }
  ...
  function barrelMuzzle(tank) {
    ...
    return {
      x: base.x + Math.cos(angle) * 78,
      y: base.y + Math.sin(angle) * 78,
      angle,
    };
  }
  ...
  function moveTank(tank, direction, dt) {
    ...
    const speed = storm.active ? 92 : 118;
  ...
  function fireShell(tank) {
    ...
    tank.shellCooldown = 0.92;
    addSmoke(muzzle.x - Math.cos(muzzle.angle) * 18, muzzle.y - Math.sin(muzzle.angle) * 18, 12, "#3f3b32");
    addShake(0.18, 7);
    playCannonSound();
  }
  ...
  function fireMachineGun(tank) {
    ...
    const muzzle = barrelMuzzle(tank);
    const angle = barrelAngle(tank) + randomBetween(-0.035, 0.035);
    const speed = 980;
    bullets.push({
      x: muzzle.x,
      y: muzzle.y,
  ...
  function registerHit(tank, x, y, weapon) {
    ...
    tank.hits = Math.min(HIT_LIMIT, tank.hits + 1);
    tank.health = Math.max(0, HIT_LIMIT - tank.hits);
  ...
  function drawWorld(target) {
    ...
    sky.addColorStop(0, "#7ec7df");
    sky.addColorStop(0.44, "#f1cc86");
    sky.addColorStop(1, "#c58d4a");
    target.fillStyle = sky;
    target.fillRect(0, 0, WORLD.width, WORLD.height);

    target.save();
    target.globalAlpha = 0.78;
    target.fillStyle = "#ffe5a3";
    target.beginPath();
    target.arc(1280, 118, 58, 0, Math.PI * 2);
    target.fill();
    target.restore();

    drawFarDunes(target, 520, "#dfb16a", 0.72, 0.0021);
    drawFarDunes(target, 590, "#c99351", 0.78, 0.0032);
    drawFarDunes(target, 670, "#a9723d", 0.9, 0.0042);

    target.fillStyle = "#b98242";
    target.beginPath();
    ...
    target.save();
    target.strokeStyle = "rgba(255, 224, 158, 0.24)";
    target.lineWidth = 2;
    for (let x = -80; x < WORLD.width + 100; x += 115) {
      const y = terrainY(x) + randomBetween(18, 58);
      target.beginPath();
      target.moveTo(x, y);
      target.quadraticCurveTo(x + 55, y + 18, x + 126, y - 4);
      target.stroke();
    }
    target.restore();
  }
  ...
  function drawTank(target, tank) {
    ...
    target.beginPath();
    target.ellipse(0, 30, 64, 13, 0, 0, Math.PI * 2);
    target.fill();

    target.fillStyle = "#263238";
    roundedRect(target, -58, -2, 116, 36, 18);
    target.fill();

    target.fillStyle = "#161d20";
    roundedRect(target, -51, 10, 102, 19, 11);
    target.fill();

    for (let i = -39; i <= 39; i += 19) {
      target.fillStyle = "#596a6e";
      target.beginPath();
      target.arc(i, 19, 9, 0, Math.PI * 2);
      target.fill();
    ...
    roundedRect(target, -50, -28, 100, 38, 8);
    target.fill();

    target.fillStyle = "rgba(255,255,255,0.16)";
    roundedRect(target, -35, -23, 52, 8, 4);
    target.fill();

    target.save();
    target.translate(0, -34);
    target.fillStyle = tank.color;
    roundedRect(target, -30, -18, 62, 30, 14);
    target.fill();
    target.fillStyle = "rgba(0,0,0,0.18)";
    target.fillRect(-22, -10, 16, 4);
    ...
    roundedRect(target, 8, -5, 78, 10, 4);
    target.fill();
    target.fillStyle = "#111719";
    roundedRect(target, 72, -7, 18, 14, 5);
    target.fill();
    target.restore();

    target.save();
    target.rotate(tank.facing === 1 ? -0.24 : Math.PI + 0.24);
  ...
  function drawStormOverlay(target, width, height, strength, intense) {
    ...
    const dustGradient = target.createLinearGradient(0, 0, width, height);
    dustGradient.addColorStop(0, `rgba(216, 164, 84, ${baseAlpha * strength})`);
    dustGradient.addColorStop(0.55, `rgba(231, 190, 117, ${baseAlpha * 0.66 * strength})`);
    dustGradient.addColorStop(1, `rgba(108, 78, 45, ${baseAlpha * 0.42 * strength})`);
    target.fillStyle = dustGradient;
  ...
  function drawPeriscopeView(target, viewport, tank, enemy) {
    ...
    sky.addColorStop(0, "#7ec5d7");
    sky.addColorStop(0.58, "#e4bd78");
    sky.addColorStop(1, "#a26b38");
    target.fillStyle = sky;
    ...
    const crossX = viewport.x + viewport.w * 0.5;
    const crossY = viewport.y + viewport.h * 0.5;
    target.strokeStyle = "rgba(40, 49, 45, 0.76)";
    target.lineWidth = 2;
    target.beginPath();
    target.moveTo(crossX - 28, crossY);
    target.lineTo(crossX + 28, crossY);
    target.moveTo(crossX, crossY - 20);
    target.lineTo(crossX, crossY + 20);
    target.stroke();
  }
  ...
  function drawCockpitFrame(target, width, height, viewport, tank) {
    ...
    target.fill();
    drawGauge(target, width * 0.225, height * 0.765, Math.PI * (0.15 + tank.health / HIT_LIMIT * 0.7), tank.color);

    target.fillStyle = "#26343a";
    roundedRect(target, width * 0.39, height * 0.68, width * 0.23, height * 0.17, 7);
    target.fill();
    drawAmmoBars(target, width * 0.425, height * 0.72, width * 0.16, height * 0.08, tank.mgAmmo / MG_LIMIT);
    ...
    target.fillText(`${tank.health}/15`, width * 0.225, height * 0.85);
    target.fillText(`${tank.mgAmmo}/30`, width * 0.505, height * 0.85);
    target.fillText(`${Math.round(tank.elevation)}°`, width * 0.78, height * 0.85);
  ...
  function updateHud() {
    ui.redHealth.style.width = `${(redTank.health / HIT_LIMIT) * 100}%`;
    ui.blueHealth.style.width = `${(blueTank.health / HIT_LIMIT) * 100}%`;
    ui.redHits.textContent = `命中 ${redTank.hits}/15`;
    ui.blueHits.textContent = `命中 ${blueTank.hits}/15`;
    ui.redAmmo.textContent = `机枪 ${redTank.mgAmmo}/30`;
    ui.blueAmmo.textContent = `机枪 ${blueTank.mgAmmo}/30`;
    ui.redAngle.textContent = `炮管 ${Math.round(redTank.elevation)}°`;
    ui.blueAngle.textContent = `炮管 ${Math.round(blueTank.elevation)}°`;

    const stormSeconds = Math.max(0, Math.ceil(storm.nextAt - gameTime));
    if (storm.active) {
      ui.stormStatus.textContent = "沙尘暴来袭";
      ui.stormStatus.classList.add("is-active");
    } else {
      ui.stormStatus.textContent = `沙尘暴 ${stormSeconds} 秒后`;
